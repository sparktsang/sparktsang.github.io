...
description: |
  大道至簡，毋須故弄玄虛。洗淨艱澀術語，毋庸半點根基。
  本筆記由淺入深，帶你察視智械之基理，網絡之源流、算法之骨架、盡覽堂奧。
...
谷岡廣樹、康鑫《深度學習入門教室：6 堂基礎課程 + Python 實作練習，Deep Learning、人工智慧、機器學習的理論和應用全圖解》
...
五十年代的首波人工智能風潮，將手動計算改以規則自動執行，進行規則庫研究，並開發了許多自動機  
...
八十年代的次波人工智能風潮發展出大量知識庫，收錄人類經驗和知識常識實現更接近人類的人工智能 knowledge-base  
...
九十年代 Hinton 等研究出即使類神經網絡有四層以上，也能解決梯度消失和局部最佳化問題，2012 年其團隊運用深層類神經網絡使影像辨識準確度遙遙領先，令深度學習萬眾矚目，形成第三波人工智能風潮  
深度學習同時還應用於影像自動生成、自然語言生成、自動翻譯、機器人控制等，2016 年 AlphaGo 擊敗世界頂尖棋士  

機器學習分為：  
監督式學習 supervised learning：給予資料和正確解答來學習  
除類神經網絡外，包括單純貝氏分類器、邏輯迴歸、決策樹、支援向量機  
可讓電腦高速準確進行分類  
...
量度誤差的，稱為損失函數 loss function，常見形式包括均方誤差與交叉熵誤差  
均方：$$E = \frac{1}{2} \sum_{k} (y(k) - t(k))^2$$；1/2 可打消微分後的常數 2  
交叉熵：$$E = - \sum_{k} \left(t(k) \log_e(y(k))\right)$$    
可用於無法簡單算差值的情況如區別貓狗  
y 為機器判定；t 為正確解答  
...
Dropout 將輸入隨機重置為 0 防止過適，0.2 表示兩成輸入會重置  
...
loss 以 categorical_crossentropy 指定，即交叉熵作為損失基準中 Early stopping 設定學習到某種程度可提早結束的功能  
