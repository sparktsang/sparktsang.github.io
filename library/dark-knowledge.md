...
title: "Notes on <i>Dark knowledge: How Machines Think, Learn and Reshape Our future?</i>"
short_title: "Dark knowledge: How Machines Think, Learn and Reshape Our future?"
nav_title: Dark knowledge
title_obvious: false
description: Demystify the "dark knowledge" generated by machine learning. These notes summarize how AI neural networks perceive the world, and what this monumental technological leap means for major industries and the future of human labour.
...
The Five Tribes of Machine Learning (and their a priori models):  
The Symbolists: independently works out decision logic (causation)  
The Bayesians: infers the probability of causes from outcomes (a cause does not necessarily produce an effect)  
The Analogizers: quantifies similarity using the nearest-neighbor method (there may be no such thing as a cause)  
The Connectionists: imitates the working principles of neurons in the human brain (hidden correlations are unfathomably deep)  
The Evolutionaries: imitates natural evolution to seek the optimal answer (none, starting from zero through trial and error)  
...
There is also fierce competition among machine learning frameworks. The main commercial value of open source is to attract more programmers to use one's own programming framework, thereby nurturing an ecosystem. The free-rider problem cannot be completely solved, and the author believes blockchain technology may be able to solve it.  
...
Autonomous driving is equivalent to a robot that can move at high speed. It requires four things: perception, decision-making, planning, and control. At present, only control technology is mature; perception, for example, is still constrained by image pixels and spatial resolution.  
...
The traditional automobile industry's supply chain runs from the immediate consumer to the automaker, then to the Tier 1 suppliers, then to the Tier 2 suppliers, and so on. Autonomous driving will break this stable supply chain, turning what was once a static ecosystem into a dynamic one.  
...
Signal mechanism: artificial neural networks use a consistent weighting system, whereas the signal strength and activation thresholds of each neuron in the human brain vary.  
